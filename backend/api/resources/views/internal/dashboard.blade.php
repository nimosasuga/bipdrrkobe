...
<div class="shell">
    ...
    <main class="main">
        ...
        <p class="muted">Ringkasan funnel diagnosis, conversion, prioritas teknis, pipeline, dan kontrol follow-up sales.</p>

        <section class="section">
            <div class="metrics">
                <article class="metric"><div class="metric-label">TOTAL LEADS</div><div class="metric-value">{{ number_format($totalLeads) }}</div><div class="metric-sub">Lead assessment yang tersimpan.</div></article>
                <article class="metric"><div class="metric-label">STARTED → LEAD</div><div class="metric-value">{{ number_format($startedToLeadRate, 1) }}%</div><div class="metric-sub">Conversion diagnosis dimulai menjadi lead.</div></article>
                <article class="metric"><div class="metric-label">LEAD → WHATSAPP</div><div class="metric-value">{{ number_format($leadToAssessmentRate, 1) }}%</div><div class="metric-sub">Lead yang melanjutkan request assessment.</div></article>
                <article class="metric"><div class="metric-label">HOT LEADS</div><div class="metric-value">{{ number_format($leadPriority['hot']) }}</div><div class="metric-sub">Prioritas follow-up tertinggi saat ini.</div></article>
            </div>
        </section>

        <section class="section">
            <div class="section-head"><h2 class="section-title">Diagnosis Funnel</h2><div class="section-note">Unique session count · test event dikecualikan</div></div>
            <div class="funnel">
                @php $funnelItems = [
                ...
                @foreach($funnelItems as [$label, $value])<article class="funnel-card"><div class="funnel-label">{{ strtoupper($label) }}</div><div class="funnel-value">{{ number_format($value) }}</div></article>@endforeach
            </div>
        </section>

        <section class="section">
            <div class="section-head"><h2 class="section-title">Lead Priority</h2><a class="section-link" href="{{ route('internal.leads.index') }}">ALL LEADS →</a></div>
            <div class="priority">
        ...
        <section class="section">
            <div class="section-head"><h2 class="section-title">Sales Pipeline</h2><div class="section-note">Klik tahap untuk membuka lead pada status tersebut</div></div>
            <div class="pipeline-summary">
                <article class="pipeline-summary-card"><div class="pipeline-summary-label">OPEN PIPELINE</div><div class="pipeline-summary-value">{{ number_format($openPipeline) }}</div></article>
                <article class="pipeline-summary-card"><div class="pipeline-summary-label">CLOSED</div><div class="pipeline-summary-value">{{ number_format($closedPipeline) }}</div></article>
                <article class="pipeline-summary-card"><div class="pipeline-summary-label">WON RATE</div><div class="pipeline-summary-value">{{ number_format($wonRate, 1) }}%</div></article>
            </div>
            @php $pipelineItems = [
            ...
            <div class="pipeline">
                @foreach($pipelineItems as [$label, $class, $value, $caption])
                    <a class="pipeline-card {{ $class }}" href="{{ route('internal.leads.index', ['status' => $class]) }}"><div class="pipeline-stage">{{ strtoupper($label) }}</div><div class="pipeline-count">{{ number_format($value) }}</div><div class="pipeline-caption">{{ $caption }}</div></a>
                @endforeach
            </div>
        </section>

        <section class="section">
            <div class="section-head"><h2 class="section-title">Follow-up Control</h2><a class="section-link" href="{{ route('internal.leads.index') }}">ALL LEADS →</a></div>
            <div class="follow-control">
                <a class="follow-card overdue" href="{{ route('internal.leads.index', ['followup' => 'overdue']) }}"><div class="follow-label">OVERDUE</div><div class="follow-value">{{ number_format($followUpControl['overdue']) }}</div><div class="follow-copy">Jadwal follow-up sudah lewat dan pipeline masih terbuka.</div></a>
                <a class="follow-card today" href="{{ route('internal.leads.index', ['followup' => 'today']) }}"><div class="follow-label">DUE TODAY</div><div class="follow-value">{{ number_format($followUpControl['today']) }}</div><div class="follow-copy">Follow-up berikutnya jatuh tempo hari ini, zona Asia/Jakarta.</div></a>
                <a class="follow-card unscheduled" href="{{ route('internal.leads.index', ['followup' => 'unscheduled']) }}"><div class="follow-label">UNSCHEDULED</div><div class="follow-value">{{ number_format($followUpControl['unscheduled']) }}</div><div class="follow-copy">Pipeline masih terbuka tetapi belum memiliki jadwal berikutnya.</div></a>
            </div>
        </section>

        <section class="section">
            <div class="section-head"><h2 class="section-title">Lead Prioritas Terbaru</h2><a class="section-link" href="{{ route('internal.leads.index') }}">LIHAT SEMUA →</a></div>
            <div class="table-wrap">
                @if($recentLeads->isEmpty())
                    <div class="empty">Belum ada lead tersimpan.</div>
                @else
                    <table class="lead-table">
                        <thead><tr><th>PRIORITY</th><th>SALES STATUS</th><th>COMPANY / SITE</th><th>PIC</th><th>MODEL</th><th>FLEET</th><th>HEALTH</th><th>REASON</th><th>CAPTURED</th><th></th></tr></thead>
                        <tbody>
                            @foreach($recentLeads as $lead)
                                ...
                                <tr>
                                    ...
                                    <td>{{ optional($lead->created_at)->timezone('Asia/Jakarta')->format('d M Y H:i') }}</td>
