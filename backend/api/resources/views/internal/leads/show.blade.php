...
<header class="top">
    <div class="top-inner">
        ...
        <div class="actions">
            <a class="btn" href="{{ route('internal.dashboard') }}">← Dashboard</a>
...
<main class="main">
    <div class="eyebrow">PHASE 8 · LEAD DETAIL · SALES FOLLOW-UP</div>
    <div class="hero">
        <div>
            <h1 class="title">{{ $lead->perusahaan }}</h1>
            <div class="sub">{{ $lead->kota ?: '-' }} · {{ $lead->nama }} · captured {{ optional($lead->created_at)->format('d M Y H:i') }}</div>
        </div>
        <div class="badges">
    ...
    <div class="grid">
        <div class="stack">
            <section class="card">
                ...
                @if($issues->isNotEmpty())
                    <div class="reason"><strong>Masalah yang dilaporkan</strong>
                        <ul class="issues">
                ...
                @if($lead->ai_summary)
                    <div class="summary">
                        <div class="summary-label">DIAGNOSTIC SUMMARY</div>
                        <p>{{ $lead->ai_summary }}</p>
                    </div>
                @endif
            </section>
            ...
            <section class="card">
                ...
                <p class="card-note">Setiap perubahan sales status dicatat sebagai activity trail.</p>
                    ...
                    <div class="timeline">
                        @foreach($lead->activities as $activity)
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-meta">{{ optional($activity->created_at)->format('d M Y H:i') }} · {{ $meta['user_name'] ?? 'system' }}{{ !empty($meta['user_role']) ? ' · '.strtoupper($meta['user_role']) : '' }}</div>
        ...
        <aside class="stack">
            <section class="card">
                <h2>Sales Follow-up Status</h2>
                <p class="card-note">Perbarui tahapan lead berdasarkan tindakan nyata yang sudah dilakukan.</p>
                <form class="form" method="POST" action="{{ route('internal.leads.status', $lead) }}">
                    @csrf
                    <label class="label">Status
                        <select class="select" name="status" required>
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" @selected($currentStatus === $status)>{{ $statusLabels[$status] }}</option>
                            @endforeach
                        </select>
                    </label>
                    <label class="label">Catatan follow-up
                        <textarea class="textarea" name="note" maxlength="1000" placeholder="Contoh: PIC sudah dihubungi. Menunggu jadwal site assessment."></textarea>
                    </label>
    ...
    <div class="footer">DRRKOBE · Internal Sales Intelligence · Lead traceability</div>
</main>
</body>
</html>
